...
local trackedEntities = {} -- Отслеживаем уже подключенные существа
local currentEntityLogs = {}
...
local function trackEntityAnimations(entityName, animator, sourceType)
    if trackedEntities[entityName] then
    ...
    local connection = animator.AnimationPlayed:Connect(function(track)
        if track and track.Animation then
            ...
            if animId then
                ...
                addToGlobalLog(fullId, entityName)
            end
        end
    end)
    
    entityAnimationListeners[entityName] = connection
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if character then
                ...
                local animator = humanoid and humanoid:FindFirstChildOfClass("Animator")
                if animator then
                    trackEntityAnimations(player.Name, animator, "Player")
                end
            end
        end
    end
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Humanoid") then
            ...
            if animator and not isPlayerCharacter then
                local entityName = character.Name .. "_" .. tostring(character)
                trackEntityAnimations(entityName, animator, "NPC")
            end
        end
    end
end
...
local function setupNPCTracking()
    -- Отслеживаем появление новых объектов в Workspace
...
local function RefreshEntitiesList()
    ...
    for _, entity in pairs(entities) do
        ...
        ViewButton.MouseButton1Click:Connect(function()
            ...
            -- Создаем временный слушатель для этого существа
            local tempConnection = entity.Animator.AnimationPlayed:Connect(function(track)
                if track and track.Animation then
                    local animId = track.Animation.AnimationId:match("rbxassetid://(%d+)")
                    if animId then
                        local fullId = "rbxassetid://" .. animId
            ...
            entityAnimationListeners["temp_" .. entity.Name] = tempConnection
...
-- Периодическое сканирование для новых существ (каждые 3 секунды)
