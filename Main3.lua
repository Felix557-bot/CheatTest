...
local animationNames = {} -- {[animationId] = name}
...
-- Функция для получения названия анимации
local function getAnimationName(animationId)
    -- Пытаемся получить название из разных источников
    local id = animationId:match("rbxassetid://(%d+)")
    if id then
        -- Проверяем, есть ли уже сохраненное название
        if animationNames[animationId] then
            return animationNames[animationId]
        end
        
        -- Пытаемся получить название через ContentProvider
        local success, result = pcall(function()
            local contentProvider = game:GetService("ContentProvider")
            local assetInfo = contentProvider:GetAssetInfo(id)
            if assetInfo and assetInfo.Name then
                return assetInfo.Name
            end
        end)
        
        if success and result then
            animationNames[animationId] = result
            return result
        end
    end
...
local function addToSaves(animationId, sourceName)
    ...
    local animName = getAnimationName(animationId)
    ...
    if existingRow then
        ...
        AnimRow.Size = UDim2.new(1, 0, 0, 45)
        ...
        local NameLabel = Instance.new("TextLabel", AnimRow)
...
local function updateCurrentAnimation(animId)
    if animId then
        ...
        local animName = getAnimationName(animId)
        SpectateAnimLabel.Text = "Current Animation: " .. (animName or "Unknown") .. " (" .. (animIdNum or "Unknown") .. ")"
    else
        SpectateAnimLabel.Text = "Current Animation: None"
    end
end
