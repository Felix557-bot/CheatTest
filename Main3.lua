...
-- Кнопки для LoggerSaves
local SavesButtonBar = Instance.new("Frame", SavesFrame)
SavesButtonBar.Size = UDim2.new(1, 0, 0, 40)
SavesButtonBar.Position = UDim2.new(0, 0, 0, 0)
SavesButtonBar.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
SavesButtonBar.ZIndex = 2

local CopyAllButton = Instance.new("TextButton", SavesButtonBar)
...
local ClearAllButton = Instance.new("TextButton", SavesButtonBar)
...
-- Смещаем список вниз, чтобы не перекрывать кнопки
SavesList.Padding = UDim.new(0, 5)
local savesContentFrame = Instance.new("Frame", SavesFrame)
savesContentFrame.Size = UDim2.new(1, 0, 1, -40)
savesContentFrame.Position = UDim2.new(0, 0, 0, 40)
savesContentFrame.BackgroundTransparency = 1

-- Переносим все будущие элементы в savesContentFrame
local function moveSavesChildren()
    for _, child in pairs(SavesFrame:GetChildren()) do
        if child ~= SavesButtonBar and child ~= savesContentFrame and child ~= SavesList then
            child.Parent = savesContentFrame
        end
    end
end
...
local function addToSaves(animationId, sourceName)
    ...
    if animationRows[animationId] then
        ...
        local AnimRow = Instance.new("Frame", savesContentFrame)
...
local function clearAllSaves()
    ...
    animationRows = {}
    SavesFrame.CanvasSize = UDim2.new(0, 0, 0, 40)
end
...
local function spectateEntity(target)
    ...
    if targetChar and targetChar:FindFirstChild("HumanoidRootPart") then
        ...
        -- Сохраняем connection для очистки
