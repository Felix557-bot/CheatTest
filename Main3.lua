...
local animationRows = {}
local entityAnimationListeners = {}
local trackedEntities = {}
local entityAnimationHistory = {} -- Сохраняем историю анимаций для каждого существа {[entityId] = {[animationId] = count}}
local currentEntityId = nil -- ID текущего открытого существа
...
-- Функция для добавления анимации в глобальные логи
...
-- Подключаем кнопки
CopyAllButton.MouseButton1Click:Connect(copyAllIds)
ClearAllButton.MouseButton1Click:Connect(clearAllSaves)

-- Функция для добавления анимации в историю существа
local function addToEntityHistory(entityId, animationId)
    if not entityAnimationHistory[entityId] then
        entityAnimationHistory[entityId] = {}
    end
    
    if not entityAnimationHistory[entityId][animationId] then
        entityAnimationHistory[entityId][animationId] = 0
    end
    entityAnimationHistory[entityId][animationId] = entityAnimationHistory[entityId][animationId] + 1
end

-- Функция для отображения истории анимаций существа
local function displayEntityHistory(entityId)
    ...
    -- Отображаем все анимации из истории
    for animId, count in pairs(entityAnimationHistory[entityId]) do
...
-- Функция для добавления анимации в логи локального игрока
...
-- Функция для добавления анимации в глобальный лог и историю
local function addToGlobalLog(animationId, sourceName, entityId)
    addToSaves(animationId, sourceName)
    if entityId then
        addToEntityHistory(entityId, animationId)
    end
end

-- Функция для отслеживания анимаций конкретного существа
local function trackEntityAnimations(entityId, entityName, animator, sourceType)
    ...
    local connection = animator.AnimationPlayed:Connect(function(track)
        if track and track.Animation then
            ...
            if animId then
                ...
                addToGlobalLog(fullId, entityName, entityId)
            end
        end
    end)
    
    entityAnimationListeners[entityId] = connection
    trackedEntities[entityId] = true
end

-- Функция для поиска и отслеживания всех существ в игре
local function scanAndTrackAllEntities()
    -- Отслеживаем всех игроков
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if character then
                ...
                local animator = humanoid and humanoid:FindFirstChildOfClass("Animator")
                if animator then
                    local entityId = "Player_" .. player.Name
                    trackEntityAnimations(entityId, player.Name, animator, "Player")
                end
            end
        end
    end
    
    -- Отслеживаем всех NPC в Workspace
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Humanoid") then
            ...
            if animator and not isPlayerCharacter then
                local entityId = "NPC_" .. character.Name .. "_" .. tostring(character)
                trackEntityAnimations(entityId, character.Name, animator, "NPC")
            end
        end
    end
end

-- Функция для отслеживания новых персонажей игроков
local function setupPlayerTracking()
    Players.PlayerAdded:Connect(function(player)
        -- Отслеживаем когда у игрока появляется персонаж
...
-- Функция для отслеживания новых NPC
...
-- Функция для обновления списка существ (для вкладки Entities)
local function RefreshEntitiesList()
        ...
        if obj:IsA("Humanoid") then
            ...
            if animator and not isPlayerCharacter then
                table.insert(entities, {
                    ...
                    Character = character,
                    Player = nil,
                    EntityId = "NPC_" .. character.Name .. "_" .. tostring(character)
                })
            end
        end
    end
    ...
    for _, entity in pairs(entities) do
        ...
        -- Считаем количество анимаций в истории
        local animCount = 0
        if entityAnimationHistory[entity.EntityId] then
            for _ in pairs(entityAnimationHistory[entity.EntityId]) do
        ...
        NameLabel.Text = entity.Name .. " (" .. entity.Type .. ")" .. (animCount > 0 and " [" .. animCount .. " anims]" or "")
        ...
        ViewButton.MouseButton1Click:Connect(function()
            ...
            displayEntityHistory(currentEntityId)
...
-- Spectate функции (оставляем без изменений)
...
ExitSpectateButton.MouseButton1Click:Connect(exitSpectate)
    ...
    Animator.AnimationPlayed:Connect(function(track)
        if track and track.Animation then
            ...
            if animId then
                ...
                addToGlobalLog(fullId, "You", "LocalPlayer")
            end
        end
    end)
end
...
-- Запускаем отслеживание всех существ
scanAndTrackAllEntities()
setupPlayerTracking()
setupNPCTracking()

-- Периодическое сканирование для новых существ
...
-- Отслеживаем появление новых персонажей у локального игрока
